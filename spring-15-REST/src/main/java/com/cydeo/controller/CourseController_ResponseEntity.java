...
public class CourseController_ResponseEntity {
    ...
    public ResponseEntity <List<CourseDTO>> getAllCourses(){
        ...
        return ResponseEntity
                .status(HttpStatus.ACCEPTED) // returns 202
    ...
    public ResponseEntity<CourseDTO> getCourseById(@PathVariable("id") long courseId){
        return  ResponseEntity.ok(courseService.getCourseById(courseId)); // Ok returns 200
    }

    @GetMapping("/category/{name}")
    public ResponseEntity <List<CourseDTO>> getCoursesByCategory(@PathVariable ("name") String name){
